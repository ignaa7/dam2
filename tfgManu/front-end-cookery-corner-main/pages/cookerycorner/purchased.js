...
import { getPurchasedRecipes, searchPurchasedRecipes } from '@/api/purchasePetitions';
//#endregion

const Purchased = ({ recipes, pages }) => {
    //#region Elements
    ...
    const onChangePage = async () => {
        if (searching) {
            const obj = await searchPurchasedRecipes(page, searchWords, token);
            setRecipesList(obj.recipes);
        } else {
            const obj = await getPurchasedRecipes(page, token);
            setRecipesList(obj.recipes);
        }
    };
    ...
    const search = async (search) => {
        ...
        if (search === '') {
            ...
            try {
                const obj = await searchPurchasedRecipes(page, search, token);
                setRecipesList(obj.recipes);
                setTotalPages(Math.ceil(obj.totalRecipes / 10));
                setPage(1);
                setSearching(true);
...
export async function getServerSideProps({ req }) {
    try {
        ...
        const obj = await getPurchasedRecipes(1, token);
